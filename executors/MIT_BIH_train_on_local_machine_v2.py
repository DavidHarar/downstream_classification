...
def trainer(seed, 
    ...
    train_generator = MIT_BIH_DataGen(data_path=data_path, readings=splits['train'], batch_size=batch_size, num_steps = training_steps)
    validation_generator = MIT_BIH_DataGen(data_path=data_path, readings=splits['test'], batch_size=batch_size, num_steps = testing_steps)

    # create a model 
    # ------------------------------------------------------------------------------------
    # IMMPORTANT NOTE: MIT-BIH DATA CONTAINS ONLY TWO LEADS (and hence num_inputs=2)
    # ------------------------------------------------------------------------------------
    if depth == 1:
        model = DownstreamInception(dropout, scale, num_inputs).to(device)
    else:
        model = DownstreamInceptionResnet(dropout, scale, depth, num_inputs).to(device)
    ...
    # get best model
    if depth == 1:
        best_model = DownstreamInception(dropout, scale, num_inputs=2).to(device)
    else:
        best_model = DownstreamInceptionResnet(dropout, scale, depth, num_inputs=2).to(device)
    if eval_metric == 'recall_for_precision':

...
class DownstreamInception(nn.Module):
    def __init__(self, dropout, scale, num_inputs, num_classes=1):
...
class DownstreamInceptionResnetBlock(nn.Module):
    ...
    def forward(self, X):
        ...
        out = out.reshape(out.shape[0], self.num_inputs, 450)
        out = self.relu(out+X) 
...
class DownstreamInceptionResnet(nn.Module):
    def __init__(self, dropout, scale, depth, num_inputs, num_classes=1):
